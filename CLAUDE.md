...
- `app.py` — Lambda 入口點 `lambda_handler`，路由 submit/status/async worker，含圖片擷取邏輯（三層防禦 + 序號完整性驗證）
...
- `job_store/` — S3 儲存套件（OOP 架構）
  ...
  - S3 key 格式：`jobs/{job_id}/{job_id}_results.json`、`jobs/{job_id}/{job_id}_logs.json`
...
2. **背景執行**：Playwright Chromium 擷取圖片 → 結果與 debug log 寫入 S3
   ...
   - **三層防禦機制**防止重複/遺漏：
     - Layer 1：關閉彈窗後主動驗證已消失，消除 race condition
     - Layer 2：偵測 stale src（彈窗 src 與前一張相同時輪詢等待更新）
     - Layer 3：第一輪結束後識別重複/失敗索引，重新爬取
   - 去重 → 按檔名編號遞增排序
   - **序號完整性驗證**：排序後檢查 1 到 max 是否完整，針對缺漏序號滾動到對應元素重新爬取並驗證 URL
